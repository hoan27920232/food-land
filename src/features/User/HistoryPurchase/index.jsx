...
function DropDownItem({ order }) {
  ...
  return (
    <div className="order-item" style={{ marginBottom: "15px" }}>
      ...
      <Collapse
        isOpened={isExpanded}
        expandStyles={{ transition: "all 0.5s ease" }}
      >
        <div
          className="block w-full overflow-x-auto"
          style={{ border: "1px dashed #dddddd", borderTop: "none" }}
        >
            ...
            <tbody>
              {order.items.map((item, index) => (
                <tr key={index}>
                  ...
                  <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
                    {formatCurrency(item.soluong * item.sanpham.DonGia)}
                  </td>
                </tr>
              ))}
            </tbody>
  ...
  useEffect(() => {
    async function fetchOrderList() {
      try {
        ...
        const actionMe = await getAllOrder(params).then((response) => {
            ...
            <div class="row category-row">
              ...
              >
                <div className="well" style={{ overflow: "auto" }}>
                  <h2 style={{ marginLeft: "15px" }}>Lịch sử mua hàng</h2>
                  <div className="form-horizontal">
                    {orderList.map((order) => {
                      return <DropDownItem order={order} />;
                    })}
                  </div>
                </div>
              </div>
