  ...
  reducers: {
    ...
    addToCard(state, action) {
      ...
      if (itemIndex >= 0) {
        if (
          ...
          state.cartItems[itemIndex].SoLuong + 1
        ) {
          state.cartItems[itemIndex].cartQuantity += action.payload.quantity;
        } else {
          state.cartItems[itemIndex].cartQuantity =
            state.cartItems[itemIndex].SoLuong;
        }
      } else {
        let tempProduct = {
          ...action.payload,
    ...
    getTotals(state, action) {
      let { total, quantity } = state.cartItems.reduce(
        (cartTotal, cartItem) => {
          const { DonGia, cartQuantity, GiamGia } = cartItem;
          let itemTotal = 0;
          if (GiamGia > 0) {
            itemTotal = DonGia * (1 - GiamGia / 100) * cartQuantity;
          } else {
            itemTotal = DonGia * cartQuantity;
          }
